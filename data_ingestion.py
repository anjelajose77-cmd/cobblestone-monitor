...
def get_ttf_price():
    """Metric 1: TTF front_month gas price"""
    try:
        ttf=yf.Ticker("TTF=F")
        hist=ttf.history(period="2d")
        price=round(hist["Close"].iloc[-1],2)
        return{"metric":"TTF Front-Month", "value": price, "unit": "€/MWh","status":"ok"}
    except Exception as e:
        return{"metric":"TTF Front-Month", "value": None, "unit": "€/MWh", "status":f"error:{e}"}
def get_carbon_price():
    """Metric 3: EU ETS carbon price via Yahoo Finance"""
    try:
        eua = yf.Ticker("CO2.L")
        hist= eua.history(period="2d")
        price = round(hist["Close"].iloc[-1],2)
        return{"metric":"EU ETS Carbon (EUA)", "value":price, "unit":"€/tonne", "status": "ok"}
    except Exception as e:
        return{"metric": "EU ETS Carbon (EUA)", "value":"None", "unit":"€/tonne","status":f"error:{e}"}
...
def get_german_power():
    ...
    try:
        ...
        return{"metric":"German Power DA", "value": avg_price, "unit":"€/MWh", "status":"ok"}
    except Exception as e:
        return{"metric":"German Power DA","value": 88.0, "unit": "€/MWh", "status": f"fallback:{e}"}
def get_gas_storage():
    ...
    try:
        ...
        return{"metric": "EU Gas Storage Fill", "value": 45.3, "unit":"%", "status":f"using fallback:{e}"}
def get_clean_spark_spread(ttf, carbon, power):
    ...
    try:
        ...
        return{"metric":"Clean Spark Spread","value":spread,"unit":"€/MWh","status":"ok"}
    except Exception as e:
        return{"metric":"Clean Spark Spread","value": None,"unit":"€/MWh","status":f"error:{e}"}
def get_lng_sendout():
